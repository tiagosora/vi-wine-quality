...
function App() {
  ...
  const [correlation_white_wine_dataset, setCorrelationWhiteWine] = useState(null);
  ...
  useEffect(() => {
    if (red_wine_dataset == null && white_wine_dataset == null) {
      ...
      papaparse.parse('../data/winequality-red.csv', {
        ...
        complete: function (input) {
          for (let i = 0; i < input.data.length; i++) {
            if (i !== 0) {
              red_values.push({
                fixed_acidity: input.data[i][0], volatile_acidity: input.data[i][1], citric_acid: input.data[i][2], residual_sugar: input.data[i][3], chlorides: input.data[i][4], free_sulfur_dioxide: input.data[i][5], total_sulfur_dioxide: input.data[i][6], density: input.data[i][7], pH: input.data[i][8], sulphates: input.data[i][9], alcohol: input.data[i][10], quality: input.data[i][11]
              });
            }
          }
      ...
      papaparse.parse('../data/correlation_red_wine_dataset.csv', {
        download: true,
        complete: function (input) {
          let parameters = ["fixed acidity",	"volatile acidity",	"citric acid",	"residual sugar",	"chlorides",	"free sulfur dioxide",	"total sulfur dioxide",	"density",	"pH",	"sulphates",	"alcohol","quality"]
          for (let i = 0; i < input.data.length; i++) {
            if (i !== 0) {
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="content flex-grow p-8 bg-gray-100 overflow-auto">
        {selectedView === '1' && <Home/>}
        {selectedView === '2' && <BarChart white_wine_dataset={white_wine_dataset} red_wine_dataset={red_wine_dataset}/>}
        {selectedView === '3' && <div className="p-4 shadow-lg rounded-lg bg-white">Content for View 3</div>}
        {selectedView === '4' && <Correlogram correlation_red_wine_dataset={correlation_red_wine_dataset} correlation_white_wine_dataset={correlation_white_wine_dataset}/>}
